...
export function VolumeBreakdownChart({ data }: Props) {
  ...
  const total = chartData.reduce((s, d) => s + d.cartons, 0);
            ...
            <div className="grid grid-cols-2 gap-3 mt-2">
              {chartData.map((d, i) => (
                <div key={d.name} className="rounded-lg p-3 text-center" style={{ backgroundColor: `${COLORS[i % COLORS.length]}15` }}>
                  ...
                  <p className="text-sm font-bold mt-0.5">{total ? ((d.cartons / total) * 100).toFixed(1) : "0.0"}%</p>
